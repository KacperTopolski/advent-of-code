...
fn solve(threshold: i64, input: Vec<String>) -> i64 {
    ...
    for i in 0..n {
        for j in 0..m {
            for x in i.saturating_sub(20)..n.min(i+21) {
                for y in j.saturating_sub(20)..m.min(j+21) {
                    let diff = i.abs_diff(x) + j.abs_diff(y);
                    if diff > 20 {
                        continue;
                    }

                    let here = from_s[i][j] + from_e[x][y] + diff as i64;
